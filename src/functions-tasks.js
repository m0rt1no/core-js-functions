...
function getArgumentsCount(funcs) {
  return funcs.map((args) => args.length);
}
...
function getPolynom(...args) {
  return args.length
    ? (x) =>
        args.reduce(
          (sum, coef, i) => sum + coef * x ** (args.length - i - 1),
          0
        )
    : null;
}
...
function logger(func, logFunc) {
  return (...args) => {
    const argsLogg = args
      ...
      .join(',');
    logFunc(`${func.name}(${argsLogg}) starts`);
    const res = func(...args);
    logFunc(`${func.name}(${argsLogg}) ends`);
    return res;
  };
}
